...
# sin CF
dml_no_cf <- function(y, d, X, ml_method = "lasso", seed = 42) {
  ...
  fit_y <- model_y$fit(X_mat, y)
  fit_d <- model_d$fit(X_mat, d)
  
  # Predicciones in-sample
  y_hat <- model_y$pred(fit_y, X_mat)
  d_hat <- model_d$pred(fit_d, X_mat)
...
for (m in modelos) {
  ...
  # Llamada al DML SIN cross-fitting
